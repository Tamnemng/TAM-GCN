"""
ResNet + CTR-GCN On-The-Fly V15
================================
Inherits all V13 bug fixes + V14 structural fixes, then fixes 3 new bugs
found via debug_spatial_attn.py instrumentation on V14.

DEBUG FINDINGS FROM V14 (root causes of spatial attention issues):

  BUG A — Cross-attention logits near-zero (std=0.026)
    V14's sigmoid cross-attention produces attn_weights ≈ 0.5 everywhere
    because rgb_query and skel_key dot-product logits are tiny (±0.02).
    Result: cross_out = spatially-uniform average of skel_v for every pixel.
    This wastes 4/14 input channels to spatial_net with a constant map,
    and the negative mean of cross_out (-0.46) biases spatial_net output
    low for some samples → sp_attn pushed toward 0 in those regions.
    FIX: Replace sigmoid cross-attention path with V11's skel_upsample
         (ConvTranspose2d chain). This directly learns a skeleton→spatial
         mapping without dot-product, producing spatially-varying features.

  BUG B — sigma_coarse too large (8.18px on 28×28 map)
    Gaussian sigma=8px on a 28×28 map covers 2σ=16px out of 28 rows/cols.
    All 5 body parts' coarse Gaussians overlap heavily → coarse scatter
    loses body-part specificity, becomes a broad blob near image center.
    FIX: Lower init_sigma_coarse from 8.0 → 4.0 px.
         scatter_sharp (sigma≈3px) stays localized.
         scatter_coarse (sigma≈4px) now provides a moderately-spread
         overlay that is still part-distinguishable.

  BUG C — Confidence gate saturated (mean=0.97, std=0.017)
    Gate always outputs ~0.97 → skeleton always trusted at 97%.
    This means confidence gating does minimal work per-sample.
    The high saturation is partly caused by the initial bias of 1.73
    (sigmoid(1.73)≈0.85) plus the gate learning to stay high because
    the skeleton is genuinely useful for most samples.
    FIX: Lower confidence gate initial bias from 1.73 → 0.5
         (sigmoid(0.5)≈0.62 starting point — less pre-loaded confidence).
         The gate will still learn to increase when skeleton helps, but
         has more room to move and better gradient flow.

V15 keeps all V13 bug fixes:
  [V13-1] Y-axis flip: Kinect Y-up → image Y-down
  [V13-2] Normalized jitter: accel / (speed² + ε)
  [V13-3] Confidence gate input: [skel_flat, rgb_global] only
  (Note: sigmoid cross-attn is removed in V15, replaced by skel_upsample)

V15 keeps V14 structural additions:
  [V14-1] SpatialWiseGate (from V11) as multiplicative gate
  [V14-2] Multi-frame coordinate extraction (3 keyframes averaged)

Architecture change summary:
  V13/V14 spatial_net input (14 ch):
    rgb_max(1) + rgb_avg(1) + scatter_sharp(4) + scatter_coarse(4) + cross_out(4)
  V15 spatial_net input (14 ch):
    rgb_max(1) + rgb_avg(1) + scatter_sharp(4) + scatter_coarse(4) + skel_up(4)
    where skel_up is ConvTranspose upsampling of skel_grid — spatially varying.
"""
import math
import torch
...
class CrossModalAttentionV15(nn.Module):
    """Cross-Modal Attention V15.
...
class Model(nn.Module):
    """V15: V14 + 3 targeted fixes from debug_spatial_attn.py analysis.

    Bug fixes:
      [BUG A] Replace sigmoid cross-attention (spatially flat cross_out)
              with skel_upsample (ConvTranspose → spatially varying map).
      [BUG B] sigma_coarse: 8.0 → 4.0 px (reduce inter-part Gaussian overlap).
      [BUG C] Confidence gate init bias: 1.73 → 0.5 (reduce saturation ~0.97→~0.62).

    All V13 fixes retained:
      Y-flip, normalized jitter, no jitter in confidence gate.
    All V14 fixes retained:
      SpatialWiseGate, multi-frame coordinate extraction.
    """
    def __init__(self, num_class, pretrained=True, temporal_rgb_frames=5,
        ...
        self.cross_attn = CrossModalAttentionV15(
