"""
Ensemble evaluation: Weighted ResNet (skeleton+RGB) + CTR-GCN (skeleton)
...
def parse_args():
    parser = argparse.ArgumentParser(description='Ensemble Weighted ResNet + CTR-GCN')
    parser.add_argument('--ctrgcn_weights', type=str, default='./result/nucla/CTROGC-GCN.pt')
    parser.add_argument('--resnet_weights', type=str, default='./work_dir/nucla/resnet_weight/epoch65_model.pt')
    ...
    parser.add_argument('--alpha', type=float, default=1.0,
                        help='Weight for CTR-GCN. Final = ResNet + alpha * CTR-GCN')
...
def load_weight_model(weights_path, ctrgcn_weights_path, device):
    """Load the weighted ResNet model and inject frozen CTR-GCN."""
    model = WeightModel(num_class=NUM_CLASS, pretrained=False).to(device)
    
    # Inject CTR-GCN for weighting
    ...
    # Load ResNet weights
    state_dict = load_weights_robust(weights_path, device)
    # Filter to only load resnet.*, attention.* and buffer keys (skip ctrgcn.* since we loaded it above)
    ...
    model.eval()
    print(f"  ✓ Weighted ResNet loaded ({loaded} params): {weights_path}")
    return model
...
def main():
    ...
    print('=' * 60)
    print('  ENSEMBLE: Weighted ResNet + CTR-GCN')
    print('=' * 60)
    ...
    weight_model = load_weight_model(args.resnet_weights, args.ctrgcn_weights, DEVICE)
    ...
    with torch.no_grad():
        for skeleton, rgb, label in tqdm(loader, desc='Inference'):
            ...
            # Weighted ResNet: uses skeleton + rgb
            resnet_out = weight_model(skeleton, rgb)
            resnet_all_scores.append(resnet_out.cpu().numpy())
    ...
    resnet_scores = np.concatenate(resnet_all_scores)
    ...
    acc_r, cor_r, tot_r, cls_r = compute_accuracy(resnet_scores, labels)
    print_results('Weighted ResNet (Skeleton+RGB)', acc_r, cor_r, tot_r, cls_r)
    ...
    print_results(f'ENSEMBLE (Weighted ResNet + {args.alpha} * CTR-GCN)', acc_e, cor_e, tot_e, cls_e)
    ...
    print(f'  Weighted ResNet:    {acc_r*100:.2f}%')
    print(f'  Ensemble (α={best_alpha}):  {best_acc*100:.2f}%')
    improvement = (best_acc - max(acc_c, acc_r)) * 100
